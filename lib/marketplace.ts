    ...
    private _format(document: Document): IOutputSuccess {
        ...
        return {
            result: [...document.querySelectorAll('table.table_block tbody tr')]?.map(el => {
                ...
                return {
                    title: {
                        original: el.querySelector('.item_description_title')?.textContent,
                        artist: el.querySelector('.item_description_title')?.textContent?.split(/ - (.+)/)?.[0],
                        item: el.querySelector('.item_description_title')?.textContent?.split(/ - (.+)/)?.[1],
                    },
